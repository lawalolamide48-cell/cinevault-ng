...
function filter(q){
  ...
  sections.forEach(section=>{
    ...
    if(sectionCards.length)section.style.display=sectionCards.some(c=>c.style.display!=='none')?'':'none';
  });
}
...
document.querySelectorAll('[data-genre]').forEach(button=>button.addEventListener('click',()=>{
  ...
  const target=document.getElementById('movies')||document.getElementById('trending');
...
if(closeModal)closeModal.onclick=closeSearchModal;
...
document.querySelectorAll('.image-poster img').forEach(img=>img.addEventListener('error',()=>{
  const poster=img.closest('.image-poster');
  if(poster){poster.classList.add('poster-fallback');img.remove();}
},{once:true}));
