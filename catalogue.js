...
function currentSort(list){const mode=sort?sort.value:'popular';return [...list].sort((a,b)=>{if(mode==='az')return a.t.localeCompare(b.t);if(mode==='year')return a.y-b.y||a.t.localeCompare(b.t);if(mode==='newest')return b.y-a.y||b.pop-a.pop;return b.pop-a.pop||b.y-a.y})}
function render(){let list=data.filter(m=>activeType==='all'||m.type===activeType);if(genre)list=list.filter(m=>matchesGenre(m,genre));const q=norm(searchInput?searchInput.value:query);if(q)list=list.filter(m=>norm(m.t+' '+m.g.join(' ')+' '+m.y).includes(q));list=currentSort(list);grid.innerHTML=list.map(card).join('');count.textContent=`${list.length} title${list.length===1?'':'s'} found`;empty.hidden=list.length!==0;document.querySelectorAll('.image-poster img').forEach(img=>img.addEventListener('error',()=>{const p=img.closest('.image-poster');if(p){p.classList.add('poster-fallback');img.remove()}},{once:true}));}
