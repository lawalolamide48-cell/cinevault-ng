...
function card(m){const href=(m.type==='series'?'series.html':'movie.html')+'?title='+slug(m.t);const tags=m.g.filter(x=>!['Series','Nigeria','African Cinema'].includes(x)).slice(0,3).join(' · ');return `<a class="card" href="${href}" data-title="${String(m.t).replace(/"/g,'&quot;')}" data-tags="${m.g.join(' ')}"><div class="poster image-poster" data-title="${String(m.t).replace(/"/g,'&quot;')}"><img src="${m.img||''}" alt="${String(m.t).replace(/"/g,'&quot;')} ${m.y} poster" loading="lazy" referrerpolicy="no-referrer"><span>${m.badge||m.y}</span></div><div><h3>${m.t}</h3><p>${tags}${tags?' · ':''}${m.y}</p></div></a>`}
function currentSort(list){const mode=sort?sort.value:'popular';return [...list].sort((a,b)=>{if(mode==='az')return a.t.localeCompare(b.t);if(mode==='year')return b.y-a.y||b.pop-a.pop; if(mode==='newest')return b.y-a.y||b.pop-a.pop;return b.pop-a.pop||b.y-a.y})}
